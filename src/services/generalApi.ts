...
import { objectToFormData, objectToJsonData } from '@/utils/axios'
...
export default {
  ...
  getOne: async ({ url_params, filters, routeName }: generalParams) => {
    return await axiosInstance
      .get(routeTo(routeName, filters, url_params))
      .then((response) => {
        return parseResponseData(response.data)
      })
      .catch((error) => error)
